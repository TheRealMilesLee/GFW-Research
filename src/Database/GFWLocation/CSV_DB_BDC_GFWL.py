...
if os.name == 'nt':
  ...
  BeforeDomainChangeFolder = '/home/silverhand/Developer/SourceRepo/GFW-Research/Lib/BeforeDomainChange/'
...
class CSVProcessor(DataProcessor):
  ...
  def process_csv(self, file):
    readingResults = []
    with open(os.path.join(self.folder_location, file), 'r',
...
class CM_GFWL_Processor(TextProcessor):
  ...
  def process(self):
    ...
    readingResults = self.merge_results(readingResults,
                                        ['domain', 'dns_server'])
    self.db_handler.create_index([('domain', 1), ('dns_server', 1)],
                                 unique=True)  # 创建复合唯一索引
    return readingResults
  ...
  def process(self):
    ...
    for result in readingResults:
      if 'is_accessible' in result:
        if 'True' in result['is_accessible'] and 'False' in result[
            'is_accessible']:
...
if __name__ == "__main__":
  ...
  processors = [
      CM_GFWL_Processor(
          ...
          os.path.join(BeforeDomainChangeFolder, 'GFWLocation/')),
      CM_IPB_Processor(MongoDBHandler(BDC_db['China-Mobile-IPBlocking']),
                       os.path.join(BeforeDomainChangeFolder, 'IPBlocking/')),
      CT_IPB_Processor(
          MongoDBHandler(BDC_db['China-Telecom-IPBlocking']),
          os.path.join(BeforeDomainChangeFolder, 'Mac', 'IPBlocking')),
      UCD_GFWL_Processor(
          MongoDBHandler(BDC_db['UCDavis-CompareGroup-GFWLocation']),
          os.path.join(BeforeDomainChangeFolder, 'CompareGroup',
                       'GFWLocation/')),
      UCD_IPB_Processor(
          MongoDBHandler(BDC_db['UCDavis-CompareGroup-IPBlocking']),
          os.path.join(BeforeDomainChangeFolder, 'CompareGroup',
                       'IPBlocking/'))
  ]
  ...
  def process_processor(processor):
    ...
    logger.info(
        f"Processed {len(results)} records for {processor.__class__.__name__}"
    )
