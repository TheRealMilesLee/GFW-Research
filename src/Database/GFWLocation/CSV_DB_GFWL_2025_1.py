...
CM_GFWL_ADC_JAN = ADC_db['ChinaMobile-GFWLocation-2025-January']
...
def processingFile(file, mongodbOP_CM_GFWL):
  ...
  with open(file, 'r') as csvfile:
    ...
    for row in reader:
      ...
      data_dict[key]['Redirection Detected'].append(
          row['Redirection Detected'])
    ...
    for domain, value in data_dict.items():
      ...
      update_data = {
          '$addToSet': {
              ...
              'RST Detected': {
                  '$each': data['RST Detected']
              },
              'Redirection Detected': {
                  '$each': data['Redirection Detected']
              },
              'Invalid IP': {
                  '$each': data['Invalid IP']
              },
              'Error': {
                  '$each': data['Error']
              }
      ...
      logger.info(
          f'Inserting DNSPoisoning data into MongoDB with domain: {domain}')
      mongodbOP_CM_GFWL.update_one({'domain': domain},
                                   update_data,
                                   upsert=True)
...
def dump_to_mongo():
  ...
  if os.name == 'nt':
    ...
    FileFolderLocation = '/home/silverhand/Developer/SourceRepo/GFW-Research/Lib/Data-2025-1/China-Mobile/GFWLocation'
  csv_files = [
  ...
  CM_GFWL_ADC_JAN.drop()

  # Create index for domain field
  logger.info('Creating index for domain field')
  CM_GFWL_ADC_JAN.create_index('domain', unique=True)

  with concurrent.futures.ThreadPoolExecutor() as executor:
    futures = [
        executor.submit(processingFile, file, mongodbOP_CM_GFWL)
